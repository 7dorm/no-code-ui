...
exports.removeBlockAndCleanup = void 0;
function isDeletable(block) {
    return block.type !== 'component';
}
function getParentComponent(blocks, startId) {
...
function collectSubtree(blocks, rootId) {
...
function getParentImportedComponentNames(parentComponent) {
    return new Set((parentComponent.imports ?? [])
        .map(imp => imp.split('|')[2])
        .filter(Boolean));
}
function traverseParentExcludingSubtree(blocks, rootId, excluded, onComponent) {
    function dfs(id) {
        if (excluded.has(id))
            return;
        const block = blocks[id];
...
function removeBlockAndCleanup(blocks, rootId, cssBlocks) {
    ...
    for (const block of Object.values(blocks)) {
        if (!block)
            continue;
        if (subtree.has(block.id))
            continue;
        if (block.type !== 'component-instance')
            continue;
        if (block.filePath !== parentComponent.filePath)
            continue;
    ...
    if (parentComponent.imports) {
        ...
        cleanedImports[parentComponent.id] = before.filter(imp => !parentComponent.imports.includes(imp));
    }
    ...
    if (cssBlocks) {
        for (const id of subtree) {
            ...
            for (const usedId of block.uses) {
                ...
                if (!cssBlock || cssBlock.type !== 'css-class')
                    continue;
...
exports.removeBlockAndCleanup = removeBlockAndCleanup;
