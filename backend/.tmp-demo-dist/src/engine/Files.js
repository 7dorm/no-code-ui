...
function removeBlockAndCleanup(blocks, rootId) {
    ...
    for (const compId of usedComponents) {
        ...
        if (!comp)
            continue;
        const usedOutside = comp.usedIn?.some(usageId => !subtree.has(usageId));
        if (!usedOutside) {
    ...
    for (const block of Object.values(blocks)) {
        block.childrenIds = block.childrenIds.filter(id => !removedBlockIds.includes(id));
    }
    return {
        removedBlockIds,
        cleanedImports,
