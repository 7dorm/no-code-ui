  ...
  const handleCreateProject = async (projectName: string, projectType: string) => {
    try {
      ...
      if (!result.canceled && result.directoryHandle) {
        ...
        try {
          const projectDirHandle = await result.directoryHandle.getDirectoryHandle(projectName, {
            create: true
          });
  ...
  return <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <Text style={styles.title} numberOfLines={1}>NO-CODE UIIIIIIIIIIIIIIIIII</Text>
        </View>
        <View style={styles.headerRight}>
          {selectedFile?.filePath && <View style={styles.topToolbar}>
              <TouchableOpacity style={[styles.modeButton, viewMode === 'preview' && styles.modeButtonActive]} onPress={() => setViewMode('preview')}>

                <Text style={[styles.modeButtonText, viewMode === 'preview' && styles.modeButtonTextActive]}>Превью</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modeButton, viewMode === 'split' && styles.modeButtonActive]} onPress={() => setViewMode('split')}>

                <Text style={[styles.modeButtonText, viewMode === 'split' && styles.modeButtonTextActive]}>Сплит</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modeButton, viewMode === 'changes' && styles.modeButtonActive]} onPress={() => setViewMode('changes')}>

                <Text style={[styles.modeButtonText, viewMode === 'changes' && styles.modeButtonTextActive]}>Изменения</Text>
              </TouchableOpacity>
              {viewMode === 'split' && <View style={styles.splitButtons}>
                  <TouchableOpacity style={[styles.modeButton, showSplitSidebar && styles.modeButtonActive]} onPress={() => setShowSplitSidebar(prev => !prev)}>

                    <Text style={[styles.modeButtonText, showSplitSidebar && styles.modeButtonTextActive]}>Панель</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modeButton, showSplitPreview && styles.modeButtonActive]} onPress={() => setShowSplitPreview(prev => !prev)}>

                    <Text style={[styles.modeButtonText, showSplitPreview && styles.modeButtonTextActive]}>Превью</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modeButton, showSplitCode && styles.modeButtonActive]} onPress={() => setShowSplitCode(prev => !prev)}>
              ...
              <View style={styles.canvasControls}>
                <View style={styles.deviceSwitch}>
                  <TouchableOpacity style={[styles.modeButton, previewDevice === 'desktop' && styles.modeButtonActive]} onPress={() => applyCanvasPreset('desktop')}>

                    <Text style={[styles.modeButtonText, previewDevice === 'desktop' && styles.modeButtonTextActive]}>ПК</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modeButton, previewDevice === 'mobile' && styles.modeButtonActive]} onPress={() => applyCanvasPreset('mobile')}>
                ...
                <View style={styles.canvasInputs}>
                  ...
                  <TextInput value={canvasWidthInput} onChangeText={setCanvasWidthInput} onBlur={() => commitCanvasInput(canvasWidthInput, 'w')} onSubmitEditing={() => commitCanvasInput(canvasWidthInput, 'w')} keyboardType="numeric" style={styles.canvasInput} />

                  <Text style={styles.canvasLabel}>H</Text>
                  <TextInput value={canvasHeightInput} onChangeText={setCanvasHeightInput} onBlur={() => commitCanvasInput(canvasHeightInput, 'h')} onSubmitEditing={() => commitCanvasInput(canvasHeightInput, 'h')} keyboardType="numeric" style={styles.canvasInput} />

                </View>
              </View>
              {isInternalSourceFile(selectedFile.filePath) && <TouchableOpacity style={[styles.modeButton, aggressivePreviewMode && styles.modeButtonActive]} onPress={() => setAggressivePreviewMode(prev => !prev)}>
          ...
          {!projectPath && <TouchableOpacity style={[styles.selectButton, styles.createButton]} onPress={() => setCreateProjectDialogVisible(true)}>

              <Text style={styles.selectButtonText}>
                ✨ Создать проект
              </Text>
            </TouchableOpacity>}
          <TouchableOpacity style={styles.selectButton} onPress={handleSelectProject}>

            <Text style={styles.selectButtonText}>
              {projectPath ? '📁 Сменить папку' : '📁 Выбрать папку проекта'}
            </Text>
          </TouchableOpacity>
          {projectPath && <Text style={styles.projectPath} numberOfLines={1}>
              {projectPath}
            </Text>}
        </View>
      </View>
      ...
      <View style={styles.mainContent}>
        ...
        <View style={[styles.sidebar, isLeftPanelCollapsed ? styles.sidebarCollapsed : {
        width: sidebarWidth
      }]} onPointerEnter={handleLeftPanelMouseEnter} onPointerLeave={handleLeftPanelMouseLeave}>

          
          {/* Содержимое левой панели */}
          <View style={[styles.sidebarContent, isLeftPanelCollapsed && styles.sidebarContentCollapsed]}>

            <View style={styles.sidebarHeader}>
              <Text style={styles.sidebarTitle}>Файлы проекта</Text>
            </View>
            <FileTree key={`file-tree-${projectPath || 'none'}-${fileTreeVersion}`} rootPath={projectPath!} onSelectFile={handleSelectFile} selectedPath={selectedFile?.selectionKey || ''} onStartComponentDrag={payload => {
            setExternalFileDrag(null);
            setExternalComponentDrag(payload);
          }} onEndComponentDrag={() => setExternalComponentDrag(null)} onStartFileDrag={payload => {
            setExternalComponentDrag(null);
            setExternalFileDrag(payload);
          }} onEndFileDrag={() => setExternalFileDrag(null)} onUnsupportedComponentDrag={message => {
            setExternalComponentDrag(null);
            setError(message);
          }} />
        ...
        <View style={styles.content}>
          {selectedFile?.filePath ? <RenderFile filePath={selectedFile.filePath} selectedComponentName={selectedFile.componentName || null} projectPath={projectPath} viewMode={viewMode} onViewModeChange={setViewMode} showSplitSidebar={showSplitSidebar} showSplitPreview={showSplitPreview} showSplitCode={showSplitCode} canvasWidth={canvasWidth} canvasHeight={canvasHeight} canvasDevice={previewDevice} aggressivePreviewMode={aggressivePreviewMode} externalComponentDrag={externalComponentDrag} externalFileDrag={externalFileDrag} onProjectFilesChanged={() => setFileTreeVersion(v => v + 1)} onOpenFile={handleSelectFile} /> : <View style={styles.placeholder}>
      ...
      <CreateProjectDialog visible={createProjectDialogVisible} onClose={() => setCreateProjectDialogVisible(false)} onCreate={handleCreateProject} />
